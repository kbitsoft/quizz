...
class Mcq:
    ...
    def result_store(self):
        ...
        for i in range(self.num_of_q):
            sql="insert into result(user_id,test_no,qNo,reponse,answer,mark)values(%s,%s,%s,%s,%s,%s)"
            val=(user,test_no,i+1,respons[i],questions[i]['answer'],mark[i])
    ...
    def summery_result(self):
        ...
        for i in range(self.num_of_q):
            ...
            correct_answer = f"Correct Answer :{questions[i]['answer']}" 
            options_display = ""
            ...
            marks_display += f"{question}{options_display}{response}{correct_answer}\n\n"
...
try:
    with open('library/data.json') as f:
        data = json.load(f)
